...
public class ProfileActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        notificationRef = ConfigurationFirebase.setRealtimeDatabaseRef()
                .child(NameFolderFirebase.NOTIFICATIONS);
    ...
    private void acceptChatRequest() {
        contactsRef.child(senderUserID)
                ...
                .addOnCompleteListener(task -> contactsRef.child(receiverUserID)
                        .child(senderUserID)
                        .child(NameFolderFirebase.CONTACTS)
                        .setValue(Constant.SAVED)
                        .addOnCompleteListener(task1 -> {
                                ...
                                chatRequestRef.child(senderUserID)
                                    ...
                                    if (task2.isSuccessful()) {
                                        chatRequestRef.child(receiverUserID)
                                            ...
                                            currentState = Constant.CONTACT;
                                            sendMessageRequestButton.setText("Remove contact");
                                            declineMessageRequestButton.setVisibility(View.
                                                    INVISIBLE
                                            );
                                            declineMessageRequestButton.setEnabled(false);
                                        });
    ...
    private void sendChatRequest() {
        chatRequestRef.child(senderUserID)
                .child(receiverUserID)
                .child(Constant.REQUEST_TYPE)
                .setValue(Constant.SENT)
                .addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
                chatRequestRef.child(receiverUserID)
                        .child(senderUserID)
                        .child(Constant.REQUEST_TYPE)
                        .setValue(Constant.RECEIVED)
                        .addOnCompleteListener(task1 -> {
                    if (task1.isSuccessful()) {
                        ...
                        notificationRef.child(receiverUserID)
                                .push()
                                .setValue(chatRequestNotifications)
                                .addOnCompleteListener(task2 -> {
                                    if (task2.isSuccessful()) {
